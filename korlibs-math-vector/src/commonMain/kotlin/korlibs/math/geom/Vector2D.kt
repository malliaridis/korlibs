...
data class Vector2D(val x: Double, val y: Double) : IsAlmostEquals<Vector2D> {
    ...
    fun round(): Vector2D = Vector2D(kotlin.math.round(x), kotlin.math.round(y))
    fun ceil(): Vector2D = Vector2D(kotlin.math.ceil(x), kotlin.math.ceil(y))
    fun floor(): Vector2D = Vector2D(kotlin.math.floor(x), kotlin.math.floor(y))
    ...
    companion object {
        ...
        fun distance(x1: Int, y1: Int, x2: Int, y2: Int): Double =
            hypot(x1.toDouble() - x2.toDouble(), y1.toDouble() - y2.toDouble())
        ...
        fun minComponents(p1: Vector2D, p2: Vector2D): Vector2D = Vector2D(
            min(p1.x, p2.x),
            min(p1.y, p2.y)
        )
        ...
        fun maxComponents(p1: Vector2D, p2: Vector2D): Vector2D = Vector2D(
            max(p1.x, p2.x),
            max(p1.y, p2.y)
        )
