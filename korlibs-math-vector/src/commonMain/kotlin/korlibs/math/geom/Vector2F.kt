...
data class Vector2F(val x: Float, val y: Float) {
    ...
    companion object {
        ...
        fun angleArc(a: Vector2F, b: Vector2F, up: Vector2D = Vector2D.UP): Angle = Angle.fromRadians(
        ...
        fun minComponents(p1: Vector2F, p2: Vector2F): Vector2F = Vector2F(
            min(p1.x, p2.x),
            min(p1.y, p2.y)
        )
        ...
        fun maxComponents(p1: Vector2F, p2: Vector2F): Vector2F = Vector2F(
            max(p1.x, p2.x),
            max(p1.y, p2.y)
        )
