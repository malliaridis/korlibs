...
data class MPoint(
    ...
    fun floor() = setTo(kotlin.math.floor(x), kotlin.math.floor(y))
    fun round() = setTo(kotlin.math.round(x), kotlin.math.round(y))
    fun ceil() = setTo(kotlin.math.ceil(x), kotlin.math.ceil(y))
    ...
    companion object {
        @Deprecated("")
        val POOL: ConcurrentPool<MPoint> =
            ConcurrentPool<MPoint>({ it.setTo(0.0, 0.0) }) { MPoint() }
