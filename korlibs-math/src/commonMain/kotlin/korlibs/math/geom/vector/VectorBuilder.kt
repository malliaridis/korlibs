...
interface VectorBuilder {
    ...
    fun quadTo(cx: Double, cy: Double, ax: Double, ay: Double) = quadTo(
        Point(cx, cy),
        Point(ax, ay)
    )
    fun quadTo(cx: Int, cy: Int, ax: Int, ay: Int) = quadTo(Point(cx, cy), Point(ax, ay))

    fun cubicTo(c1x: Float, c1y: Float, c2x: Float, c2y: Float, ax: Float, ay: Float) = cubicTo(
        Point(c1x, c1y),
        Point(c2x, c2y),
        Point(ax, ay)
    )
    fun cubicTo(c1x: Double, c1y: Double, c2x: Double, c2y: Double, ax: Double, ay: Double) = cubicTo(
        Point(c1x, c1y),
        Point(c2x, c2y),
        Point(ax, ay)
    )
    fun cubicTo(c1x: Int, c1y: Int, c2x: Int, c2y: Int, ax: Int, ay: Int) = cubicTo(
        Point(c1x, c1y),
        Point(c2x, c2y),
        Point(ax, ay)
    )
