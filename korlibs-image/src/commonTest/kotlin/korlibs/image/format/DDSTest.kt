...
class DDSTest {
    ...
    fun dxt1() = suspendTestNoBrowser {
        val output = resourcesVfs["res/dxt1.dds"].readBitmapNoNative(props)
        val expected = resourcesVfs["res/dxt1.png"].readBitmapNoNative(props)
        assertEquals(0, output.matchContentsDistinctCount(expected))
    }

    @Test
    fun dxt3() = suspendTestNoBrowser {
        val output = resourcesVfs["res/dxt3.dds"].readBitmapNoNative(props)
        val expected = resourcesVfs["res/dxt3.png"].readBitmapNoNative(props)
    ...
    fun dxt5() = suspendTestNoBrowser {
        val output = resourcesVfs["res/dxt5.dds"].readBitmapNoNative(props)
        val expected = resourcesVfs["res/dxt5.png"].readBitmapNoNative(props)
        assertTrue { output.matchContentsDistinctCount(expected) < 7000 }
    ...
    fun dxt5_registered() = suspendTestNoBrowser {
        RegisteredImageFormats.temporalRegister(DDS, PNG) {
            val output = resourcesVfs["res/dxt5.dds"].readBitmapNoNative()
            val expected = resourcesVfs["res/dxt5.dds"].readBitmapNoNative(DDS.toProps())
            assertEquals(0, output.matchContentsDistinctCount(expected))
            //output.writeTo(LocalVfs("c:/temp/dxt5.png"))
        }
    }
